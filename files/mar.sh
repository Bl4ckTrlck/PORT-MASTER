...
su -c "$CURRENTDIR/boot-wa.sh $PATCHDATE $SOURCEROM $OUTP $CURRENTDIR" $CURRENTUSER
...
/persist.vendor.camera.model=/c\persist.vendor.camera.model=MI 6X" $lasys/system/build.prop
...
setfattr -h -n security.selinux -v u:object_r:system_file:s0 $lasys/system/etc/dualcamera.png
chown -hR root:root $lasys/system/etc/dualcamera.png

cp -f $CURRENTDIR/MIUI_DualCamera_watermark.png $laven/etc/
chmod 644 $laven/etc/MIUI_DualCamera_watermark.png
setfattr -h -n security.selinux -v u:object_r:system_file:s0 $laven/etc/MIUI_DualCamera_watermark.png
chown -hR root:root $laven/etc/MIUI_DualCamera_watermark.png
...
#VENDOR
cp -f $CURRENTDIR/fstab.qcom $laven/etc/
